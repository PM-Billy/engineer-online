...
> | 5.4 问题详情 | **H5 WebView** | 富文本/翻译/嵌套回复迭代快，前端独立发版避免 App 跟随 |
...
interface UserBrief {
  ...
  nickname: string;
  avatar: string;
  role: 'REGULAR' | 'CAR_OWNER' | 'AUTHORIZED_OWNER' | 'OFFICIAL_ENGINEER' | 'OFFICIAL_PM' | 'ADMIN';
  officialInfo?: {
...
GET    /groups                          # 圈子列表
GET    /groups/{id}                     # 圈子详情
GET    /groups/{id}/questions           # 问题列表
POST   /groups/{id}/questions           # 发布问题
...
|---------|------|------|
| 0 | 成功 | success |
| 1xxx | 通用/系统 | 参数、鉴权、限流等 |
| 2xxx | 用户/权限 | 角色、登录、注册 |
| 3xxx | 圈子模块 | Group 相关 |
| 4xxx | 问题模块 | Question相关 |
| 5xxx | 回答模块 | Answer相关 |
| 6xxx | 审核模块 | Audit相关 |
| 7xxx | 消息/推送 | Notification相关 |
| 8xxx | 文件/翻译 | File/Translate相关 |
| 9xxx | 系统内部 | 数据库、网络、第三方服务 |

**详细错误码表**:

```yaml
# 1xxx 通用
1001: PARAM_INVALID              # 参数校验失败
1002: PARAM_MISSING              # 必填参数缺失
1003: RATE_LIMITED               # 请求过于频繁（Sentinel触发）
1004: SIGN_INVALID               # 签名错误

# 2xxx 用户/权限
2001: UNAUTHORIZED               # 未登录
2002: TOKEN_EXPIRED              # Token过期
2003: TOKEN_INVALID              # Token无效
2004: FORBIDDEN                  # 无权限（角色不足）
2005: GROUP_FORBIDDEN           # 无圈子访问权限

# 3xxx 圈子
3001: GROUP_NOT_FOUND           # 圈子不存在
3002: GROUP_DISABLED            # 圈子已停用
3003: GROUP_NAME_DUPLICATE      # 圈子名称重复

# 4xxx 问题
4001: QUESTION_NOT_FOUND         # 问题不存在
4002: QUESTION_TITLE_TOO_SHORT   # 标题过短（<5字符）
4003: QUESTION_TITLE_TOO_LONG    # 标题过长（>200字符）
4004: QUESTION_ALREADY_DELETED   # 问题已删除
4005: QUESTION_CANNOT_DELETE     # 问题不可删除（已采纳回答）
4006: QUESTION_NO_POST_PERM      # 无发布权限
4007: CONTENT_SENSITIVE          # 内容包含敏感词

# 5xxx 回答
5001: ANSWER_NOT_FOUND           # 回答不存在
5002: ANSWER_ALREADY_ACCEPTED    # 回答已被采纳
5003: ANSWER_CANNOT_ACCEPT_OWN   # 不能采纳自己的回答
5004: ANSWER_SYNC_FAILED         # 同步失败
5005: ANSWER_ALREADY_ADOPTED     # 问题已采纳该回答（重复操作）

# 6xxx 审核
6001: AUDIT_REASON_REQUIRED      # 驳回原因必填
6002: AUDIT_ALREADY_PROCESSED    # 已审核，不可重复操作
6003: AUDIT_STATUS_INVALID       # 审核状态非法

# 7xxx 消息
7001: NOTIFICATION_NOT_FOUND     # 通知不存在

# 8xxx 文件/翻译
8001: FILE_TOO_LARGE             # 文件过大
8002: FILE_TYPE_INVALID          # 文件类型不支持
8003: UPLOAD_FAILED              # 上传失败
8004: TRANSLATE_FAILED           # 翻译失败

# 9xxx 系统
9001: INTERNAL_ERROR             # 内部错误（不暴露详情）
9002: DB_ERROR                   # 数据库错误
9003: CACHE_ERROR                # 缓存错误
9004: MQ_ERROR                   # 消息队列错误
9005: THIRD_PARTY_ERROR          # 第三方服务错误
```
...
- **搜索索引**: 问题/回答的全文搜索索引（如Elasticsearch不可用时的降级方案）
- **埋点数据**: 用户行为事件（后续分析用）
...
**图片处理**:
- 缩略图: `{url}?x-oss-process=image/resize,w_300,h_300,m_fill`
- 原图: 直接访问
...
Request:
{
  ...
  "contentType": "QUESTION",  // QUESTION / ANSWER / REPLY
  "sourceId": 12345          // 内容ID，用于缓存
}
...
**缓存策略**:
- Redis Key: `translate:{content_md5}:{target_lang}`
- TTL: 7天
- 内容变更时：MD5变化，自动失效旧缓存

**降级策略**:
- Google Translate API失败时，返回原文并提示 "Translation service unavailable"
- 记录失败日志，触发告警
...
> 推荐优先调用 `prompts/ 中的角色 prompt 模板，下面给出几个常见调用示例。

### 13.1 生成 Flutter 原生页面（5.2/5.3/5.5/5.6）

```
请按 prompts/Flutter页面.md 的指引（若文件不存在请参考 H5详情页.md 思路），为 PRD 05.02（圈子首页）生成 Flutter 原生页面：
...
```
请按 prompts/H5详情页.md 的指引，为 PRD 05.04 生成 Vue 3 + NuxtJS + Vant 详情页：
...
```
请按 prompts/后端接口与服务.md 的指引，为 PRD 05.03（问题发布）生成 Spring Boot 代码：
...
```
请按 prompts/数据库设计.md 的指引，为 PRD 05.02.6（Question 数据对象）生成 MySQL 建表语句：
