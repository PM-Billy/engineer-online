...
apis:
  ...
  - "PUT /api/v1/admin/questions/{id}/hide"
  - "POST /api/v1/admin/questions/{id}/push-uop"
  - "POST /api/v1/admin/questions/push-uop"
  - "PUT /api/v1/admin/questions/sort"
permission_codes: ["question:audit:any", "question:delete:any", "question:push-uop:any", "question:sort:any"]
updated: "2026-05-09"
---
...
| 字段 | 内容 |
|------|------|
| 功能描述 | 后台管理员对问题进行审核、管理（热门/隐藏/删除），支持批量操作 |
...
| E-A-06 | 公开状态 | 单选下拉 | 全部 | 公开/不公开 | 筛选列表 | `admin.question.filter.public_status` | - |
| E-A-07 | 采纳状态 | 单选下拉 | 全部 | 未采纳/已采纳 | 筛选列表 | `admin.question.filter.accept_status` | - |
| E-A-08 | 推送状态 | 单选下拉 | 全部 | 未推送/已推送/推送失败/更新失败 | 筛选列表 | `admin.question.filter.push_status` | UOP 同步状态 |
...
| E-A-11 | 车系 | 单选下拉 | 全部 | - | 筛选列表 | `admin.question.filter.vehicle_series` | 车型筛选 |
| E-A-12 | 年份 | 单选下拉 | 全部 | - | 筛选列表 | `admin.question.filter.year` | - |
| E-A-13 | VIN | 文本输入 | 空 | - | 筛选列表 | `admin.question.filter.vin` | 精确匹配 |
| E-A-14 | 发布人手机/邮箱 | 文本输入 | 空 | - | 筛选列表 | `admin.question.filter.poster_contact` | 精确搜索 |
| E-A-15 | 发布人昵称 | 文本输入 | 空 | - | 筛选列表 | `admin.question.filter.poster_nickname` | 精确搜索 |
| E-A-16 | 用户身份 | 单选下拉 | 全部 | 普通用户/官方号 | 筛选列表 | `admin.question.filter.poster_role` | - |
| E-A-17 | 搜索按钮 | 按钮 | - | - | 提交表单 | `common.btn_search` | - |
| E-A-18 | 重置按钮 | 按钮 | - | - | 提交表单 | `common.btn_reset` | 清空筛选条件 |

**页面元素清单 — 批量操作区**：

| 编号     | 元素名称   | 类型  | 约束/校验规则 | 交互行为 | i18n key                                                   | 说明                    |
| ------ | ------ | --- | ------- | ---- | ---------------------------------------------------------- | --------------------- |
| E-A-19 | 批量按钮   | 按钮  | -       | 切换状态 | `admin.question.btn.batch_mode`                            | 进入多选模式                |
| E-A-20 | 自定按钮   | 按钮  | -       | 打开弹窗 | `admin.question.btn.customize_columns`                     | 自定义列显示                |
| E-A-21 | 排序     | 按钮  | 需选中至少1条 | 提交排序 | `admin.question.btn.batch_sort`                            | 输入排序数字，正数，按排序数字由小到大排序 |
| E-A-22 | 审核通过   | 按钮  | 需选中至少1条 | 提交表单 | `common.btn_approve`                                       | 绿色                    |
| E-A-23 | 审核驳回   | 按钮  | 需选中至少1条 | 打开弹窗 | `common.btn_reject` / `admin.question.field.reject_reason` | 橙色，需填写驳回原因            |
| E-A-24 | 批量删除   | 按钮  | 需选中至少1条 | 打开弹窗 | `admin.question.btn.batch_delete`                          | 红色，二次确认               |
| E-A-25 | 推送到UOP | 按钮  | 需选中至少1条 | 提交表单 | `admin.question.btn.batch_push_uop`                        | 通过UOP问题推送接口同步到UOP     |
...
- BR-5.7-04: 隐藏操作：隐藏后问题在移动端不可见，但后台仍可管理
...
| 操作类型 | action_type | enum | 是 | APPROVE / REJECT / DELETE / HOT / UNHOT / HIDE / EDIT | - |
...
> 复用 5.2.7 中定义的 Question 状态机。后台管理操作（审核、热门、隐藏、删除）均为状态转换的触发动作。
...
- AC-5.7-08: **Given** 管理员编辑某问题的排序序号 **When** 输入正数并保存 **Then** 列表按排序序号由小到大重新排列
...
| 接口 | 方法 | 路径 | 主要参数 | 返回 | 说明 |
|------|------|------|----------|------|------|
| 问题列表 | GET | /api/v1/admin/questions | keyword, questionId, groupId, startDate, endDate, reviewStatus, publicStatus, acceptStatus, pushStatus, isHot, hasMedia, vehicleSeries, year, vin, posterPhone, posterEmail, posterNickname, posterRole, page, pageSize | PageResult\<AdminQuestionVO\> | 手机/邮箱脱敏 |
...
| 隐藏/取消隐藏 | PUT | /api/v1/admin/questions/{id}/hide | {isHidden: boolean} | - | - |
| 推送到UOP | POST | /api/v1/admin/questions/{id}/push-uop | - | {pushStatus: string} | 同步问题到UOP |
...
| 编辑问题 | PUT | /api/v1/admin/questions/{id} | AdminQuestionUpdateDTO | - | 记录变更日志 |
| 删除问题 | DELETE | /api/v1/admin/questions/{id} | - | - | 二次确认；同步 UOP |
| 查看回复面板 | GET | /api/v1/admin/questions/{id}/answers | reviewStatus?, acceptStatus?, pushStatus?, page, pageSize | PageResult\<AdminAnswerVO\> | 详见 5.8 |
