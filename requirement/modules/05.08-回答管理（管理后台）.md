...
apis:
  ...
  - "POST /api/v1/admin/answers/{id}/resync"
  - "DELETE /api/v1/admin/answers/{id}"
permission_codes: ["answer:audit:any", "answer:delete:any", "answer:resync:any"]
updated: "2026-05-09"
---
...
| 采纳状态 | 文本 | 已采纳 / 未采纳 |
| 推送状态 | 状态标签 | 已同步（绿）/ 同步失败（红）/ 未同步 | 仅官方号回答有状态变化 |
| 赞 | 数字 | - |
| 踩 | 数字 | - |
...
- BR-5.8-03: UOP 同步失败的回答，管理员可手动重新触发同步
- BR-5.8-04: 已采纳的回答不允许删除（需先取消采纳）
...
|------|------|----------|----------|----------|----------|
| EX-5.8-01 | UOP 同步失败 | UOP 接口异常 | 标记同步状态为失败 | 列表显示"同步失败" | 管理员手动重新同步 |
| EX-5.8-02 | 删除已采纳的回答 | 回答已被采纳 | 拒绝删除 | "已采纳的回答不可删除" | 需先取消采纳 |
...
> Answer 实体有两个独立状态字段：`review_status`（审核状态）和 `sync_to_uop_status`（UOP 同步状态）。审核通过后，仅当作者为官方号时才进入 UOP 同步流程；普通用户回答审核通过后无同步状态变化（保持 `NOT_SYNCED`）。
...
| Approved | Deleted | 删除 | 作者/管理员 | 移动端不可见；已采纳的回答不可删（BR-5.8-04） |

### 5.8.7.2 UOP 同步状态机（`sync_to_uop_status`，仅官方号回答）
...
stateDiagram-v2
    ...
    SyncFailed --> Synced : 管理员手动重新同步
    UpdateFailed --> Synced : 管理员手动重新同步
```
...
| NOT_SYNCED | SYNC_FAILED | UOP 接口异常（首次推送） | 系统自动 | 后台标记，告警运营 |
| SYNCED | UPDATE_FAILED | UOP 接口异常（更新推送） | 系统自动 | 后台标记，告警运营 |
| SYNC_FAILED | SYNCED | 管理员点击"重新同步" | 管理员 | 数据推送到 UOP；记录操作日志 |
| UPDATE_FAILED | SYNCED | 管理员点击"重新同步" | 管理员 | 数据推送到 UOP；记录操作日志 |
...
| Approved | SYNC_FAILED | ✓ | ✗ | 首次推送失败，等待管理员重新同步 |
| Approved | UPDATE_FAILED | ✓ | ✓ | 更新推送失败，等待管理员重新同步 |
...
- AC-5.8-03: **Given** 管理员筛选"同步失败" **When** 点击重新同步 **Then** 系统重试同步，成功后状态更新为"已同步"

**异常流程：**
- AC-5.8-04: **Given** 某回答已被采纳 **When** 管理员尝试删除 **Then** 提示"已采纳的回答不可删除"
...
| 删除回答 | DELETE | /api/v1/admin/answers/{id} | - | - | 已采纳的不可删 |
| 重新同步 | POST | /api/v1/admin/answers/{id}/resync | - | {syncToUopStatus} | 同步状态需为 SYNC_FAILED |
...
```http
POST /api/v1/admin/answers/55001/resync
Authorization: Bearer admin-jwt
```
...
```json
{
  "code": 5004,
  "message": "同步失败",
  "data": { "currentSyncStatus": "SYNCED", "expected": "SYNC_FAILED" },
...
```json
{
  "code": 5002,
  "message": "已采纳的回答不可删除",
  "data": { "answerId": 55001, "isAccepted": true },
