...
| E-04 | 标题输入框         | 文本输入    | 空                | 必填，5-4000字符                   | -                              | `ask.field.title.label` / `ask.field.title.placeholder`             | placeholder: "Enter a title"                                                                                           |
...
| E-06 | 图片/视频上传区      | 图片/视频上传 | 空                | 图片+视频合计≤9个，图片≤5MB，视频≤50MB/60s | 上传文件，上传中显示取消按钮，上传后显示移除按钮，点击可预览 | `ask.field.images.label`                                            | 支持相册和拍照；格式统一转为jpg、mp4                                                                                                  |
...
- BR-5.3-01: 标题为必填，5-4000 字符
- BR-5.3-02: 描述为选填（Optional），最多 10,000 字符
- BR-5.3-03: 图片/视频合计最多上传 9 个，图片单张 ≤ 5 MB，视频单个 ≤ 50 MB、时长 ≤ 60s。格式统一转为 jpg、mp4 后上传（遵循 GR-CONTENT-02/03）
- BR-5.3-04: 选择车辆：若圈子 `enabled_features` 包含 `vehicle` 且当前用户是车主或授权车主，则显示该字段，默认选中当前车主的默认车辆，点击弹出选择车辆界面；若不满足条件则不显示
- BR-5.3-05: 总里程：若圈子 `enabled_features` 包含 `mileage` 且当前用户是车主或授权车主，则显示该字段，必填，点击弹出填写车辆里程界面；若不满足条件则不显示
...
- BR-5.3-11: 图片/视频上传走 OSS 直传：客户端每份媒体需上传两份文件（原文件 + 缩略图），分别调 `/api/v1/upload/presign` 拿预签名 URL，前端 PUT 到 OSS，再分别调 `/api/v1/upload/confirm` 确认。上传策略：用户选择图片/视频后立即开始上传两份文件，显示上传进度；上传成功后在界面显示缩略图和删除按钮；点击发表时只提交已上传成功的 CDN URL 列表，未上传完成的不计入
- BR-5.3-12: 客户端压缩最佳实践：上传前在客户端进行压缩。原图：长边 ≤ 1200px，质量 80%，目标单张 ≤ 2MB，使用原生压缩库（Flutter: `flutter_image_compress`）。缩略图：正方形居中裁剪 450×450px，质量 75%，单独生成后上传。视频：压缩至 ≤ 50MB（分辨率 720p，码率 2Mbps），同时截取第一帧生成封面缩略图（450×450px 居中裁剪，质量 75%）。压缩后若原图仍 > 5MB 则提示用户选择更小图片
...
| EX-5.3-02 | 标题过长      | 标题达到 4000 个字符           | 阻止继续输入                  | 输入框右下显示当前字数/4000        | -         |
| EX-5.3-03 | 图片格式/大小错误 | 非允许格式或 > 5 MB           | 拒绝该图，标记失败               | "图片格式不支持/超出大小限制（≤5 MB）" | 重选        |
| EX-5.3-04 | 视频格式/大小错误 | 非 mp4 或 > 50 MB 或 > 60s | 拒绝该视频                   | "视频格式/大小/时长超出限制"        | 重选        |
...
| 接口 | 方法 | 路径 | 主要参数 | 返回 | 说明 |
|------|------|------|----------|------|------|
| 创建问题 | POST | /api/v1/groups/{groupId}/questions | QuestionCreateDTO (title, content?, media[]?, vehicleModel?, vin?, totalMileage?) | {id} | 自动进入审核队列；media 图片+视频合计最多9个 |
...
interface MediaItem {
  ...
  url: string;                // CDN URL（图片原图或视频原文件）
  coverUrl: string;           // 缩略图/封面 URL；图片 = OSS 压缩缩略图，视频 = 第一帧封面
  duration?: number;          // type=video 时必填，视频时长（秒）
}

interface QuestionCreateDTO {
  title: string;              // 必填，5-4000 字符
  content?: string;           // 选填，纯文本，≤10,000字符
  media?: MediaItem[];        // 图片+视频合计最多 9 个；按用户上传顺序排列，后端存入 `images` + `videos`
...

{
  ...
  "media": [
    {
      "type": "image",
      "url": "https://cdn.example.com/community/prod/20260426/img-001.jpg",
      "coverUrl": "https://cdn.example.com/community/prod/20260426/img-001-thumb.jpg"
    },
    {
      "type": "video",
      "url": "https://cdn.example.com/community/prod/20260426/video-001.mp4",
      "coverUrl": "https://cdn.example.com/community/prod/20260426/video-001-cover.jpg",
      "duration": 32
    },
    {
      "type": "image",
      "url": "https://cdn.example.com/community/prod/20260426/img-002.jpg"
    }
  ],
  "vehicleModel": "AION V",
  "totalMileage": "1203km"
...
{
  "code": 0,
  "data": { "cdnUrl": "https://cdn.example.com/community/prod/20260426/uuid-abc.jpg" }
}

# Step 3: 确认（缩略图）
POST /api/v1/upload/confirm
{ "cdnUrl": "https://cdn.example.com/community/prod/20260426/uuid-abc-thumb.jpg" }

# Response（缩略图）
{
  "code": 0,
  "data": { "cdnUrl": "https://cdn.example.com/community/prod/20260426/uuid-abc-thumb.jpg" }
}

# 最终媒体对象
{
  "type": "image",
  "url": "https://cdn.example.com/community/prod/20260426/uuid-abc.jpg",
  "coverUrl": "https://cdn.example.com/community/prod/20260426/uuid-abc-thumb.jpg"
}
```
