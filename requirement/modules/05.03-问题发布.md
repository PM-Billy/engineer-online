...
- BR-5.3-12: 客户端压缩最佳实践：上传前在客户端进行压缩，图片目标单张 ≤ 2MB（长边 ≤ 1920px，质量 80%），使用原生压缩库（Flutter: `flutter_image_compress`）。压缩后若仍 > 5MB 则提示用户选择更小图片。视频上传前压缩至 ≤ 50MB（分辨率 720p，码率 2Mbps）
...
| 接口 | 方法 | 路径 | 主要参数 | 返回 | 说明 |
|------|------|------|----------|------|------|
| 创建问题 | POST | /api/v1/groups/{groupId}/questions | QuestionCreateDTO (title, content?, mediaUrls[]?, vehicleModel?, vin?, totalMileage?) | {id} | 自动进入审核队列；mediaUrls 图片+视频合计最多9个 |
...
interface QuestionCreateDTO {
  ...
  mediaUrls?: string[];       // CDN URL 列表，图片+视频合计最多 9 个；API 输入层统一字段，后端按类型拆分为 `images` + `videos` 存入 DB，详情页 VO 按类型分开返回
