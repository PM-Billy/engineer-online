...
| 媒体缩略图      | 图片列表  | 图片/视频封面混排，最多显示3个；视频封面叠加播放图标 |
| 回答预览       | 嵌套卡片  | 灰色背景区域              |
| ├ 工程师头像    | 图片    |                     |
| ├ 工程师昵称    | 文本    | 如 "Wayne"           |
| ├ 品牌标签     | 标签    | 如 "GAC"，黑色边框        |
...
- BR-5.2-03: 问题卡片中的媒体缩略图（图片+视频封面）最多显示3个（横向排列），按用户上传顺序取前3个；超出3个时第3个显示 "+N" 数量提示；视频缩略图上叠加播放图标
...
| 媒体列表     | media              | MediaItem[] | 否   | 图片+视频合计最多9个，按用户上传顺序           | 结构化对象：type/url/coverUrl/duration                                  |
...
interface QuestionListVO {
  ...
  mediaPreview: MediaPreview[]; // 缩略图混排，最多 3 个（统一使用 coverUrl），按上传顺序
  author: UserBrief;
...
interface MediaPreview {
  type: 'image' | 'video';      // 媒体类型，视频缩略图上显示播放图标
  url: string;                  // 缩略图 URL（统一使用 coverUrl）
}
    ...
    "list": [
      {
        ...
        "mediaPreview": [
          {
            "type": "image",
            "url": "https://cdn.example.com/community/prod/20260420/abc-thumb.jpg"
          }
        ],
        "author": {
          "id": 67890,
