...
| 提交回答 | POST | /api/v1/questions/{id}/answers | AnswerCreateDTO (content, mediaUrls[]?, parentId?) | {id} | parentId 存在时为嵌套回复；mediaUrls 为 API 输入层统一字段，后端按类型拆分存入 `images`/`videos`，图片+视频合计最多3个 |
