...
| E-A-05 | 品牌标签 | 标签 | 仅官方号显示 | - | - | 如 "GAC"，灰色背景 |
| E-A-06 | 职位 | 文本 | 仅官方号显示 | - | `status.role.official_engineer` / `status.role.official_pm` | 如 "Product Manager" |
...
- BR-5.4-08: 删除问题需二次确认，删除后同步状态到 UOP（数据标识"已删除"），列表中带问号标识、暗色文字显示"问题已删除"
...
- BR-5.4-11: 用户可对工程师回答及其二级回复发起追问（追问即回复，是作者对回答的回复），追问走同样的发布和审核流程
...
- BR-5.4-20: 底部回复入口：根据圈子回复权限配置，没有权限的用户点击 toast 提示"仅允许xxx/xxxx进行回复"；有权限的用户点击后弹出回复输入框
- BR-5.4-13: 嵌套回复最多支持两层（回答 → 回复 → 回复的回复），`parent_id` 指向直接父回复。UI 上嵌套回复在对应父回复下方展示，二级回复不额外缩进
...
| 同步到UOP状态 | sync_to_uop_status | enum         | 否   | NOT_SYNCED / SYNCED / SYNC_FAILED              | 回答数据同步到 UOP 的状态    |
...
> 回答状态机见 `05.08-回答管理（管理后台）.md` 5.8.7 节（审核状态：Pending → Approved/Rejected；UOP 同步状态：NotSynced → Synced/SyncFailed，仅官方号回答）。
...
| 回答列表 | GET | /api/v1/questions/{id}/answers | page, pageSize | PageResult\<AnswerVO\> | 时间正序 |
| 提交回答 | POST | /api/v1/questions/{id}/answers | AnswerCreateDTO (content, mediaUrls[]?, parentId?) | {id} | parentId 存在时为嵌套回复；mediaUrls 图片+视频合计最多3个 |
...
| 翻译内容 | POST | /api/v1/translate | {text, targetLang} | {translatedText} | 结果可缓存 |
...
```json
{
  "code": 0,
  "data": {
    "translatedContent": "Hello, a yellow warning light usually indicates that...",
    "sourceLang": "zh",
    "targetLang": "en",
    "confidence": 0.98
  },
...
```json
{
  ...
  "data": { "fallbackContent": "您好，黄色故障灯..." },
  "traceId": "trace-tr-002",
  "timestamp": 1714123516789
}
```
