...
MAPPED_ACS=$(awk -F',' '!/^#/ && NR>1 && $5 != "-" {print $5}' "$MAPPING_FILE" | tr ';' '\n' | grep -E '^AC-' | sort -u)
...
for ac in $DEFINED_ACS; do
  ...
  if ! echo "$MAPPED_ACS" | grep -q "^$ac$"; then
    ...
    module_has_br=$(awk -F',' -v m="$module" '!/^#/ && $2 == m {found=1} END {print found+0}' "$MAPPING_FILE")
...
REFERENCED_TASKS=$(awk -F',' '!/^#/ && NR>1 {print $6","$7}' "$MAPPING_FILE" \
  | tr ',;' '\n' | tr -d '"' | grep -E '^[A-Z]+-' | grep -vE '^(BR|EX|AC|GR|RR|SEC|OPS|OQ|SEO)-' | sort -u)

DEFINED_TASKS=$(grep -oE '^\| (INF-[A-Z]+-[0-9A-Za-z]+|[A-Z]+-[0-9]+)' "$TASKS_FILE" \
  | sed 's/^| //' | sort -u)
