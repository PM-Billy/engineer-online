...
> - 完整 DDL 生成由 `prompts/数据库设计.md` 驱动
...
| 时间字段 | `created_at` / `updated_at` / `deleted_at` | 软删除用 `deleted_at` |
...
| `type` | VARCHAR(20) | NOT NULL | ENUM: `ENGINEER_ONLINE` |
| `description` | VARCHAR(1000) | NULL | 圈子介绍 |
| `background_color` | VARCHAR(16) | NULL | 品牌色 Hex，如 `#FF5500` |
| `cover_url` | VARCHAR(500) | NOT NULL | 封面图 OSS URL（≤3MB） |
| `access_roles` | JSON | NOT NULL | 访问角色数组，如 `["ALL"]` / `["OWNER","AUTH_OWNER"]` |
| `post_roles` | JSON | NOT NULL | 发帖角色数组 |
| `reply_roles` | JSON | NOT NULL | 回复角色数组 |
...
| `title` | VARCHAR(400) | NOT NULL | 5-400 字符 |
| `content` | TEXT | NULL | ≤10,000 字符（BR-5.3-02） |
| `images` | JSON | NULL | 图片 URL 数组（≤9 张，合并视频） |
| `videos` | JSON | NULL | 视频 URL 数组（单个 ≤50MB） |
| `vehicle_model` | VARCHAR(100) | NULL | 选中的车型 |
| `mileage` | INT | NULL | 车辆累计公里数 |
| `review_status` | VARCHAR(20) | NOT NULL | ENUM: `PENDING`/`APPROVED`/`REJECTED`/`HIDDEN` |
| `public_status` | VARCHAR(20) | NOT NULL | ENUM: `PUBLIC`/`PRIVATE`（BR-5.7 新增） |
| `is_hot` | TINYINT(1) | NOT NULL, default 0 | 热门标记 |
| `is_hidden` | TINYINT(1) | NOT NULL, default 0 | 隐藏标记 |
...
- `idx_public_status (public_status, review_status)` — 后台筛选
...
| `author_id` | BIGINT | NOT NULL, FK | - |
| `parent_id` | BIGINT | NULL, FK | NULL=顶层回答；非空=嵌套回复，仅支持一层（BR-5.4-16） |
| `content` | TEXT | NOT NULL | - |
| `images` | JSON | NULL | 图片 URL 数组 |
| `author_type` | VARCHAR(20) | NOT NULL | ENUM: `OFFICIAL`/`OWNER`/`ADMIN` |
| `review_status` | VARCHAR(20) | NOT NULL | ENUM: `PENDING`/`APPROVED`/`REJECTED`/`HIDDEN` |
| `sync_to_uop_status` | VARCHAR(20) | NOT NULL, default `NOT_SYNCED` | ENUM: `NOT_SYNCED`/`SYNCING`/`SYNCED`/`FAILED` |
...
| `vote_type` | VARCHAR(10) | NOT NULL | ENUM: `UP`/`DOWN`；同一用户对同一回答仅一条，切换则更新 |
| `created_at` | DATETIME | NOT NULL | - |
| `updated_at` | DATETIME | NOT NULL | - |
...
| `type` | VARCHAR(20) | NOT NULL | ENUM: `SYSTEM`（v1.4.0 起统一） |
...
| `action_type` | VARCHAR(40) | NOT NULL | ENUM: `APPROVE_Q`/`REJECT_Q`/`HIDE_Q`/`PUSH_UOP`/`SET_OFFICIAL`/... |
...
```
NOT_SYNCED ──push──→ SYNCING ──ok──→ SYNCED
                         │
                       error
                         ↓
                      FAILED ──resync──→ SYNCING
```
...
| APPROVED | FAILED | 审核通过但 UOP 同步失败（可重试） |
| REJECTED | NOT_SYNCED | 驳回的回答不应推送 |
...
- **软删除**：全部实体使用 `deleted_at`，MyBatis Plus 逻辑删除配置
